...
- Code comments: concise and direct, only when the WHY isn't obvious from the code itself (a hidden constraint, subtle invariant, workaround, or surprising behavior). Don't narrate what the code does, and don't write multi-paragraph rationale in-source — that belongs in the commit/PR description, not the file.
...
3. **Real third-party credentials** — Google OAuth is optional locally: `pnpm --filter chat db:seed` (`apps/chat/prisma/seed.ts`) seeds a "Tim Sandbox" user/circle, and `/signin` shows a "Sign into test sandbox" button (non-production only) that hits `apps/chat/src/app/api/dev/login` to mint a session for that seeded user without going through Google at all. Cloudinary/Giphy/YouTube/OpenAI keys are optional: when one is missing outside production, the corresponding call site returns a real-shaped canned response instead of failing, so local dev works without acquiring them. See `apps/chat/src/lib/cloudinary.ts:uploadImage` and `apps/realtime-server/src/lib/media-fetchers.ts` / `open-ai.ts:callOpenAI` — if you see a fixed gif/video/reply/image locally instead of a live one, that's this fallback, not a bug.
